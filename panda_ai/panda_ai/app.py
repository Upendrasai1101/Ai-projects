...
from search_tool    import search, fetch_google_news_rss, is_time_sensitive, detects_future_intent, get_upcoming_dates, extract_sources
from file_processor import process_file, process_mixed_files, allowed_file, UPLOAD_DIR, set_groq_key
...
# ── V8.2: Give file_processor a Groq key for audio/video transcription ──────
# Pass the first available key. file_processor uses it only for Whisper calls.
# The chat rotation pool (GROQ_KEYS) is unaffected — chat still rotates normally.
if GROQ_KEYS:
    set_groq_key(GROQ_KEYS[0])
    print(f"✅ file_processor: Groq Whisper key injected (...{GROQ_KEYS[0][-6:]})")
else:
...
def get_cache(q):
    """Retrieve cache entry from session-isolated storage."""
    # Get session-scoped cache from Flask session if available
    try:
        if 'user_cache' not in session:
            session['user_cache'] = {}
        
        cache_store = session['user_cache']
        k = cache_key(q)
        
        with _cache_lock:
            if k in cache_store:
                age = time.time() - cache_store[k]["ts"]
                if age < CACHE_TTL:
                    print(f"🟢 Cache HIT ({int(age)}s): {q[:40]} [Session: {session.get('user_id', 'anon')}]")
                    return cache_store[k]["context"]
                del cache_store[k]
...
def set_cache(q, ctx):
    """Store cache entry in session-isolated storage."""
    try:
        if 'user_cache' not in session:
            session['user_cache'] = {}
        ...
        with _cache_lock:
            cache_store[cache_key(q)] = {"context": ctx, "ts": time.time()}
            print(f"💾 Cache SET: {q[:40]} [Session: {session.get('user_id', 'anon')}]")
        
        # Mark session as modified to persist
...
def build_system_prompt(dc, memory_ctx, has_search, search_attempted):
    # V8.4: Calculate live year at runtime
    current_year = datetime.now(pytz.timezone("Asia/Kolkata")).year
    
    memory_section = f"\n\n{memory_ctx}" if memory_ctx else ""
    
    # V8.4: Convert to f-string with dynamic year injection
    identity = (
    ...
    widget_instructions = (
        ...
        "Example response for a location question:\n"
        "Hyderabad is a major city in South India, known for its rich history and cuisine.\n"
        "SHOW_MAP: Hyderabad, Telangana, India\n\n"
        ...
        "Example response for a data question:\n"
        "Here is the information you requested about browser usage worldwide.\n"
        "SHOW_CHART: Global browser market share 2024\n\n"
...
def get_news():
    ...
    try:
        ...
        cards   = []
        for r in results:
            if len(r.get("title","")) < 10: continue
            summary = r.get("summary","")
            if len(summary) > 130: summary = summary[:127] + "..."
            cards.append({"title":r["title"],"summary":summary,
...
def chat():
    try:
        ...
        clean_reply, widget_type, widget_query = detect_widget(reply)
