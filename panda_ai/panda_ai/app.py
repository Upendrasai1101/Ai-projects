...
def chat():
    ...
    try:
        # Search with original message (no language instruction)
        search_results = duckduckgo_search(user_message)

        # Build prompt - add language instruction for AI only
        if search_results:
            prompt = f"{user_message}\n\n[SEARCH RESULTS]\n{search_results}"
        else:
            prompt = user_message

        # Add language instruction at end (doesn't affect search)
        if lang_instruction:
            prompt = f"{prompt}\n\n[{lang_instruction}]"
