...
GROQ_KEYS = [k for k in GROQ_KEYS if k]  # remove None keys
if not GROQ_KEYS:
...
GROQ_MODELS = [
    "llama3-70b-8192",   # primary — high accuracy
    "llama3-8b-8192",    # fallback — faster
]
...
# ── Main Search (Search-First Logic) ──
...
# ── Groq with Key Rotation + Model Fallback ──
def ask_groq(messages):
    if not GROQ_KEYS:
        raise Exception("No Groq API keys configured")

    # Try each model (70b first, then 8b)
    for model in GROQ_MODELS:
        keys_to_try = list(GROQ_KEYS)
        random.shuffle(keys_to_try)  # spread load across keys
        ...
        for key in keys_to_try:
            ...
            try:
                ...
                if res.status_code == 429:
                    print(f"Groq rate limit: key={key_label} model={model} — rotating key...")
                    continue

                # Auth error — skip this key
                if res.status_code == 401:
                    print(f"Groq auth error: key={key_label} — skipping")
                    continue
...
# ── Master AI: Groq Primary → OpenRouter Fallback ──
def ask_ai(messages):
    try:
    ...
    try:
        ...
        if search_results:
            prompt = (
                ...
                f"[LIVE SEARCH CONTEXT — GROUND TRUTH]\n{search_results}\n\n"
                f"Instructions: Carefully analyze the search results above. "
                f"Answer the user's question using ONLY the search context. "
                f"Be factual, concise, and cite source titles where relevant."
            )
            system_content = (
                f"You are Panda AI — a highly accurate, real-time AI assistant. Today is {today}.\n\n"
                "STRICT RULES:\n"
                "1. LIVE SEARCH CONTEXT = GROUND TRUTH. Always prioritize over training data.\n"
                "2. Current events, scores, positions — use ONLY the search context provided.\n"
                "3. NEVER say 'I don't have real-time info' when search context is given.\n"
                "4. NEVER mention training cutoff or knowledge limitations.\n"
                "5. If search context doesn't answer the question, say so clearly and honestly.\n"
                "6. Be friendly, clear, and concise.\n"
                "7. Respond fully in the language requested by the user."
            )
        else:
            ...
            system_content = (
                ...
                "Answer from your training knowledge, but always prefix with: "
