# ─────────────────────────────────────────────────────────────
# FILE: app.py  ·  Panda AI V8.2
# ─────────────────────────────────────────────────────────────
# V8.2 CHANGES (additive only — all core logic untouched):
#   • Debug block: adds HF_TOKEN check at startup
#   • After GROQ_KEYS are loaded, calls file_processor.set_groq_key()
#     with the first available key so audio/video transcription works.
#   • /upload route: unchanged signature, but file_processor now
#     calls cloud APIs instead of local libraries.
#   • /health: reports hf_token and cloud_processor status.
#   • Future Intelligence Layer: detects user intent for upcoming
#     events and uses datetime.date.today() to filter future-dated
...
import pytz
import os, requests, json, re, random, time, hashlib, base64

from search_tool    import search, fetch_google_news_rss, is_time_sensitive, detects_future_intent, get_upcoming_dates
...
def build_system_prompt(dc, memory_ctx, has_search, search_attempted):
    ...
    identity = (
        f"You are Panda AI V8.2, a smart and friendly AI assistant built in India.\n"
