...
# ── Expand abbreviations so search gets better results ──
def expand_query(query):
    """Expand common abbreviations to full forms for better search accuracy."""
    q = query
    expansions = [
        # Political titles — avoid matching PM-Kisan, CM-related schemes
        ...
        (r'\bcm\s+of\s+([a-z\s]+)',       r'Chief Minister of \1'),
...
def search_web(query):
    ...
    query_variations = [
        f"{expanded} {dt['month_year']}",
        f"latest {expanded} {dt['year']}",
        f"{re.sub(r'who is |what is |the ', '', expanded, flags=re.IGNORECASE).strip()} {dt['year']}",
    ]
    ...
    try:
        ...
        if search_context:
            system_content = (
                ...
                "RULES:\n"
                "1. You receive LIVE SEARCH CONTEXT from Bamboo Engine below.\n"
                "2. First check: is the context RELEVANT to the user question?\n"
                "3. If RELEVANT and answer is found: answer confidently, cite the source title.\n"
                "4. If context is about a DIFFERENT topic or answer not found:\n"
                "   Use your own knowledge to answer. Do NOT say 'current data not available'\n"
                "   unless you genuinely have no knowledge of the topic.\n"
                "5. IGNORE: ads, cookie popups, nav menus, unrelated scheme names.\n"
                "6. NEVER hallucinate names, positions, or scores.\n"
                "7. Be concise, friendly, and direct.\n"
                "8. Respond in the language requested by the user."
            )
            prompt = (
                f"DATE: {dt['anchor']}\n\n"
