...
def get_web_search(query: str) -> list:
    ...
    Layers:
    1. Primary: GNews API using GNEWS_API_KEY from environment variables
    2. Secondary: Google News RSS via feedparser with the current year appended
    3. Fallback: DuckDuckGo Lite via DDGS().text(..., backend='lite', max_results=5)
    """
    ...
    try:
        ...
        if not api_key:
            print("GNews API skipped: GNEWS_API_KEY is not set")
        elif GNews is None:
            print("GNews API skipped: gnews package is unavailable")
        else:
            ...
            if response.ok:
                ...
                for item in payload.get("articles", []) or []:
                    ...
                    results.append({
                        "title": title,
                        "summary": clean_text(f"{title}. {description}" if description else title, 500),
                        "url": url,
        ...
        print(f"GNews API error: {exc}")

    # Layer 2: Google News RSS
    try:
        rss_query = year_tagged_query
        encoded_query = requests.utils.quote(rss_query)
    ...
    # Layer 3: DuckDuckGo Lite fallback
    try:
