# --- FILE: search_tool.py --- V8.3 Studio
"""
search_tool.py — Panda AI V8.3 Studio

Main + Backup Architecture (V8.3 — LAYER INTERCHANGE):
  - Primary Layer: gnews library with 5-second strict timeout (fast, direct API)
  - Backup Layer: Google News RSS (triggered only when gnews fails/times out)
  - Behavior: gnews first (5s timeout) → if timeout/fail, trigger RSS multi-strategy

Deep Crawl Mode (time-sensitive queries):
  - News fetch → top 3 URLs → trafilatura deep extract (1500 chars each)
  - IST timezone via pytz — fully dynamic, no hardcoded dates

Quick Mode (factual/general queries):
  - News snippets only — fast, lightweight
  - No date forced for identity/factual queries

V8.3 CHANGES (additive only — accuracy_plot logic UNTOUCHED):
  ✅ Layer Interchange: gnews as Primary with 5-second timeout
  ✅ Graceful Fallback: RSS triggers immediately on timeout/exception
  ✅ Timeout enforcement: strict 5s limit on gnews fetch request

V8.2 CHANGES (additive only):
...
V8.1 CHANGES (additive only — scoring/accuracy logic UNTOUCHED):
  ...
  ✅ Today + 7 Days priority filter: after news fetch, results are
...
# ════════════════════════════════════════
# SOURCE 1: PRIMARY LAYER — gnews multi-strategy
# Tries multiple query strategies with 5-second timeout per attempt
# ════════════════════════════════════════
def fetch_gnews_primary_robust(query: str, dt: dict, max_items: int = 10, timeout: float = 5.0) -> list:
    """
    Try gnews primary with multiple query strategies until results found or timeout.
    Each attempt has a strict 5-second timeout.
    Returns empty list if all strategies fail or timeout.
    """
    strategies = build_search_queries(query, dt)
    for i, q in enumerate(strategies):
        print(f"gnews Primary attempt {i+1}/{len(strategies)}: '{q[:70]}' (timeout={timeout}s)")
        results = fetch_gnews_primary(q, max_items=max_items, timeout=timeout)
        if results:
            print(f"✅ gnews Primary success: attempt {i+1} → {len(results)} entries")
            return results
        if i < len(strategies) - 1:
            time.sleep(0.1)
    print("gnews Primary: all strategies returned 0 entries or timed out")
    return []

def fetch_google_news_rss_robust(query: str, dt: dict, max_items: int = 5) -> list:
    """
    Try multiple query strategies on RSS backup until results found.
    Only triggered when gnews primary fails/times out.
    """
    strategies = build_search_queries(query, dt)
    for i, q in enumerate(strategies):
        print(f"RSS Backup attempt {i+1}/{len(strategies)}: '{q[:70]}'")
        results = fetch_google_news_rss_backup(q, max_items=max_items)
        if results:
            print(f"✅ RSS Backup success: attempt {i+1} → {len(results)} entries")
            return results
        if i < len(strategies) - 1:
            time.sleep(0.2)
    print("RSS Backup: all strategies returned 0 entries")
    return []

# ════════════════════════════════════════════════════════════════
# BACKWARD COMPATIBILITY WRAPPER
# For legacy code that calls fetch_google_news_rss() directly
# ════════════════════════════════════════════════════════════════
def fetch_google_news_rss(query: str, max_items: int = 10) -> list:
    """
    BACKWARD COMPATIBILITY WRAPPER — V8.3
    Legacy function for direct RSS fetching (used by /news endpoint).
    Returns RSS results directly without gnews layer.
    """
    results = []
    try:
        ...
        print(f"RSS Legacy: {rss_url[:110]}")

        feed = feedparser.parse(rss_url)
        if not feed.entries:
            print("RSS Legacy: 0 entries")
            return []
        ...
        print(f"RSS Legacy: {len(results)} entries")
    except Exception as e:
        print(f"RSS Legacy error: {e}")

    return results

# ════════════════════════════════════════
# SOURCE 1: PRIMARY LAYER — gnews library
# Strict 5-second timeout on fetch request
# Falls back to RSS if timeout or error occurs
# ════════════════════════════════════════
def fetch_gnews_primary(query: str, max_items: int = 10, timeout: float = 5.0) -> list:
    """
    Primary layer using gnews library with strict timeout.
    Fetches top 10 news articles (language='en', country='IN').
    If timeout or error occurs, returns empty list (RSS will be triggered as backup).
    Uses threading-based timeout for portability across all systems.
    """
    results = []
    result_container = {'data': None, 'error': None}
    
    def fetch_with_gnews():
        try:
            ...
            result_container['data'] = news_items
            
        except Exception as e:
            result_container['error'] = str(e)
    
    try:
        print(f"📰 gnews Primary: attempting for query '{query[:50]}...' (timeout={timeout}s)")
        
        # Start fetch in a separate thread with timeout
        import threading
        fetch_thread = threading.Thread(target=fetch_with_gnews, daemon=True)
        fetch_thread.start()
        fetch_thread.join(timeout=timeout)
        
        # Check if thread is still alive after timeout
        if fetch_thread.is_alive():
            print(f"⏱️ gnews Primary: TIMEOUT after {timeout}s — falling back to RSS")
            return []
        
        # Check for errors
        if result_container['error']:
            print(f"gnews Primary error: {result_container['error']}")
            print("   Falling back to Google News RSS...")
            return []
        
        news_items = result_container['data']
        if not news_items:
            print("gnews Primary: 0 results")
            return []
        ...
        for item in news_items[:max_items]:
            ...
            # Build summary from title + description (consistent with RSS mapping)
        ...
        print(f"✅ gnews Primary: {len(results)} entries fetched within {timeout}s")
        
    except Exception as e:
        print(f"gnews Primary error: {e}")
        print("   Falling back to Google News RSS...")
    
    return results

# ════════════════════════════════════════
# SOURCE 1.5: BACKUP LAYER — Google News RSS
# Triggered ONLY when gnews primary fails or times out
# Fetches top 5 news articles with fallback strategies
# ════════════════════════════════════════
def fetch_google_news_rss_backup(query: str, max_items: int = 5) -> list:
    """
    Backup layer using Google News RSS.
    Only triggered when gnews primary layer fails or times out.
    Uses robust multi-strategy approach.
    Returns top 5 news articles with same field structure as primary layer.
    """
    results = []
    try:
        print(f"📰 Google News RSS Backup: attempting for query '{query[:50]}...'")
        
        clean_q = re.sub(r'["\']', '', sanitize_query(query))
        encoded = requests.utils.quote(clean_q)
        rss_url = f"https://news.google.com/rss/search?q={encoded}&hl=en-IN&gl=IN&ceid=IN:en"
        print(f"RSS Backup: {rss_url[:110]}")

        feed = feedparser.parse(rss_url)
        if not feed.entries:
            print("RSS Backup: 0 entries")
            return []
        ...
        print(f"RSS Backup: {len(results)} entries")
        
    except Exception as e:
        print(f"RSS Backup error: {e}")
...
# ════════════════════════════════════════
def get_web_search(query: str) -> list:
    """
    Multi-layer web search with dynamic year context.

    Layers:
    1. Google News RSS via requests + feedparser with the current year appended
    2. DuckDuckGo Lite via duckduckgo_search
    3. Google Search fallback via googlesearch-python if prior layers yield nothing
    """
    ...
    year_tagged_query = cleaned_query if re.search(r'\b\d{4}\b', cleaned_query) else f"{cleaned_query} {current_year}"

    results = []

    # Layer 1: Google News RSS
    try:
        encoded_query = requests.utils.quote(year_tagged_query)
    ...
    # Layer 2: DuckDuckGo Lite
    try:
        from duckduckgo_search import DDGS

        ddgs = DDGS()
        ddg_items = ddgs.text(keywords=cleaned_query, backend="lite", max_results=5) or []
        for item in ddg_items:
            if isinstance(item, dict):
                title = (item.get("title") or item.get("href") or "").strip()
            ...
            results.append({
                "title": title,
                "summary": clean_text(f"{title}. {snippet}" if snippet else title, 500),
                "url": url,
                "published": "",
        ...
        print("DuckDuckGo fallback unavailable: duckduckgo_search is not installed")
    except Exception as exc:
        print(f"DuckDuckGo fallback error: {exc}")

    # Layer 3: googlesearch-python fallback
    try:
        from googlesearch import search

        for url in search(cleaned_query, num_results=5, stop=5, pause=1.0):
            results.append({
                "title": url,
                "summary": clean_text(url, 300),
                "url": url,
                "published": "",
        ...
        print("Google Search fallback unavailable: googlesearch-python is not installed")
    except Exception as exc:
        print(f"Google Search fallback error: {exc}")

    return []


def search(query: str, enrich_articles: bool = True) -> str:
    # ── Step 0: Empty query validation (safety guard) ──
    if not query or not query.strip():
        return "No search results found due to an empty query."
    ...
    # ── Step 1: PRIMARY LAYER — gnews with 5-second timeout (multi-strategy) ──
    gnews_results = fetch_gnews_primary_robust(query, dt, max_items=10, timeout=5.0)

    # ── Step 1.5: BACKUP LAYER — Google News RSS (only if gnews fails/times out) ──
    if not gnews_results:
        print("⚠️ gnews Primary returned no results — triggering RSS Backup Layer...")
        rss_results = fetch_google_news_rss_robust(query, dt, max_items=5)
        if rss_results:
            print(f"✅ RSS Backup success: {len(rss_results)} entries")
            all_results.extend(rss_results)
        else:
            print("❌ RSS Backup also returned no results")
    else:
        print(f"✅ Using gnews Primary results: {len(gnews_results)} entries")
        all_results.extend(gnews_results)

    # ── Step 2: V8.1 — Today+7 priority filter (separate layer) ──
    all_results = _priority_sort(all_results, dt["week_dates"])

    # ── Step 2b: V8.2 — Future Intelligence Layer filter ──
    if future_intent and all_results:
        all_results = filter_upcoming_events(all_results, query)

    # ── Step 3: Mode-based enrichment (UNCHANGED logic) ──
    if temporal and all_results and enrich_articles:
        print(f"⚡ Deep crawl mode: extracting full article bodies...")
        all_results = deep_crawl_top_urls(all_results, max_crawl=DEEP_CRAWL_URLS)
    elif all_results and enrich_articles:
        print("📄 Quick mode: using news summaries")

    # ── Step 4: Wikipedia (CONDITIONAL — Dynamic Source Exclusion) ──
    # If query is time-sensitive (temporal is True), skip Wikipedia entirely
    # to prioritize live sports and news updates
    if not temporal:
        ...
        print("⏭️ Time-sensitive query: Wikipedia excluded to prioritize live news/sports")
    ...
    # ── Strict Context Re-ordering (Prioritization) ──
    # Separate news-based array entities from Wikipedia results
    # Force hard structural re-ordering: news_results ALWAYS at top (index 0 onwards)
    news_results = [r for r in all_results if r.get("source") != "Wikipedia"]
    wiki_results = [r for r in all_results if r.get("source") == "Wikipedia"]
    final_ordered_results = news_results + wiki_results
    
    if wiki_results:
        print(f"📊 Prioritization: {len(news_results)} news sources pushed to top, {len(wiki_results)} Wikipedia pushed to bottom")

    # ── Build context ──
    # Priority: deep-crawled body > snippet summary > title
    # ACCURACY / SCORING LOGIC UNTOUCHED
    blocks = []
    current_year = dt["year"]  # Dynamic year from IST context
    
    for i, r in enumerate(final_ordered_results[:10], 1):
        title     = r.get("title","")
        ...
        # ── V8.4: Dynamic Priority Tagging ──
        # Inject priority tags based on freshness and source type
        if current_year in str(published):
            priority_tag = f"[REAL-TIME FRESH UPDATE — YEAR {current_year} — STRICT PRIORITY]"
        elif "Wikipedia" in source:
            priority_tag = "[HISTORICAL BACKUP — USE ONLY IF LIVE NEWS IS NOT AVAILABLE]"
        else:
            priority_tag = "[GENERAL CONTEXT]"

        blocks.append(
            f"[{i}. {title} | {source} | {published} {tag} {priority_tag} | {url}]\n{content}"
        )
...
# ════════════════════════════════════════
# SOURCE EXTRACTION FOR FRONTEND (V8.3)
# ════════════════════════════════════════
def extract_sources(search_context: str) -> list:
    """
    Parse source URLs and payloads from the search context string.
    Returns list of {"title": ..., "url": ..., "source": ..., "content": ...} dicts.
    Used by app.py to pass sources to frontend.
    
    Expected format: [N. Title | Source | Date TAG | URL]\nContent here
    """
    sources = []
    # Match pattern: [N. Title | Source | Date TAG | URL]\nContent
    # This captures the full block including the content payload
    blocks = re.findall(
        r'\[(\d+)\.\s(.+?)\s\|\s(.+?)\s\|.*?\|\s(https?://[^\]]+)\]\n+([^\[]*?)(?=\n\[|$)',
        search_context,
        re.DOTALL
    ...
    for num, title, source, url, content in blocks:
        if title and len(title) > 5:
            # Clean and extract meaningful content payload
            clean_content = clean_text(content.strip(), limit=300) if content else ""
            
            sources.append({
                "title":  title.strip(),
                "source": source.strip(),
                "url":    url.strip(),
                "content": clean_content,  # Include actual text payload
            })
    
    print(f"Extracted {len(sources)} sources with content payloads")
