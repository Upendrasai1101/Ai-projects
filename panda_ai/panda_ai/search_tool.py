...
Sources:
  ...
  2. trafilatura      → article body extraction (best-effort)
  3. Wikipedia API    → always works, rich factual content
Performance:
  - ThreadPoolExecutor for parallel article fetching
  - Strict 10-second timeout on all network calls
...
# ── Request timeout — strict 10s to prevent 504s ──
REQUEST_TIMEOUT = 10
...
# ── Query expansion for better search accuracy ──
...
# ── SOURCE 2: trafilatura article body (10s timeout, silent fail) ──
def _fetch_one_article(url):
    try:
        res = requests.get(
            url,
            headers={"User-Agent": get_ua(), "Accept": "text/html;q=0.9,*/*;q=0.8"},
        ...
        return url, clean_text(text or "", 700)
    except Exception:
        return url, ""
    ...
    try:
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            ...
            for fut in as_completed(futures, timeout=REQUEST_TIMEOUT + 2):
                try:
                    url, body = fut.result(timeout=1)
                    if body:
                        body_map[url] = body
                        print(f"  Article: {len(body)} chars from {url[:50]}")
                except Exception:
                    pass
    except FuturesTimeout:
        print("Article fetch: overall timeout")
    except Exception as e:
...
def search(query, enrich=True):
    """
    Returns formatted context string for Groq.
    Never raises — always returns str.
    """
    ...
    if not rss:
        rss = fetch_google_news_rss(f"{expanded} {dt['year']}")

    # 2. Enrich with article bodies (parallel, 10s timeout)
    if rss and enrich:
    ...
    # 3. Wikipedia (always add for factual grounding)
    ...
    for i, r in enumerate(all_results[:5], 1):
        content = r.get("body") or r.get("summary") or r.get("title", "")
        block   = (
