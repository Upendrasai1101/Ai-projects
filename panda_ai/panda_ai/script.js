...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (ttsBtn) {
    ttsBtn.addEventListener('click', function() {
      ...
      this.style.opacity = ttsEnabled ? '1' : '0.5';
    });
  }
  ...
  if (langSelect) {
    langSelect.addEventListener('change', function() {
      // Language preference is stored and used when sending messages
    });
  }

  // ──── ORATOR BUTTON (SIDEBAR) ────
  const oratorBtn = document.getElementById('orator-btn');
  if (oratorBtn) {
    oratorBtn.addEventListener('click', function() {
      openModal('orator-modal');
    });
  }
