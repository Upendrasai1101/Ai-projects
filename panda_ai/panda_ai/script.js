...
function getBotReplyText(data){
    const candidates=[data?.reply,data?.response,data?.answer,data?.text];
    for(const value of candidates){
        if(typeof value === 'string' && value.trim()) return value.trim();
        if(value !== undefined && value !== null && value !== '') return String(value).trim();
    }
    return "I couldn't generate a response based on the search context. Please rephrase or try again.";
}
...
function appendMessage(role,content,searched,fileBadge){
    ...
    if(!normalizedContent && role==='ai'){
        content = "I couldn't generate a response based on the search context. Please rephrase or try again.";
    } else if(!normalizedContent && role==='user'){
        return null;
    }
...
async function uploadAndAnalyze(){
    ...
    try{
        ...
        if(data.error) appendMessage('ai','⚠️ '+data.error,false,null);
            ...
            const replyText = getBotReplyText(data);
            currentMessages.push({role:'ai',content:replyText});persistSessions();
            appendMessage('ai',replyText,data.searched||false,data.file_type||'File');
...
async function sendMessage(overrideText){
    ...
    try{
        ...
        if(data.error) appendMessage('ai','⚠️ '+data.error,false,null);
            ...
            const replyText = getBotReplyText(data);
            currentMessages.push({role:'ai',content:replyText});persistSessions();
            const msgEl=appendMessage('ai',replyText,data.searched||false,null);
