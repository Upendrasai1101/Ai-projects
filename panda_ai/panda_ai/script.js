...
async function loadNews() {
  ...
  try {
    // Use dedicated /news endpoint — faster and more accurate
    const res = await fetch('/news', { method:'GET' });
    if (!res.ok) throw new Error('Server error');
    const data = await res.json();
    if (data.news && data.news.length > 0) {
      newsList.innerHTML = data.news.map(headline => {
        const safe = headline.replace(/"/g,'&quot;').replace(/'/g,'&#39;');
        return `<div class="news-item" data-text="Tell me more about: ${safe}" onclick="quickSendText(this)">📌 ${headline}</div>`;
      }).join('');
      if (data.date) {
        newsList.innerHTML += `<div style="color:#555;font-size:10px;padding:4px 8px;text-align:right;">Updated: ${data.date}</div>`;
      }
    } else {
      newsList.innerHTML = '<div style="color:#888;font-size:11px;padding:6px">No news found. Try again later!</div>';
    }
  } catch(e) {
